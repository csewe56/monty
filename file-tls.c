...
void find_func(char *opcode, char *value, int line_number, int format)
{
    ...
    if (flag == 1)
        handle_instruction_error(3, line_number, opcode);
}
 ...
 * if 1 nodes will be entered as a queue.
 */
void call_fun(op_func func, char *opcode, char *value, int line_number, int format)
{
    stack_t *node;
    int flag;
    int i;
    ...
    if (strcmp(opcode, "push") == 0)
    {
        ...
        if (value == NULL)
            handle_instruction_error(5, line_number);
        for (i = 0; value[i] != '\0'; i++)
        {
